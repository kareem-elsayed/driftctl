...
func TestS3BucketSupplier_Resources(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantAlert: alerter.Alerts{"aws_s3_bucket": []alerter.Alert{alerter.Alert{Message: "Ignoring aws_s3_bucket from drift calculation: Listing aws_s3_bucket is forbidden.", ShouldIgnoreResource: true}}},
			wantErr:   false,
		},
	}
	for _, tt := range tests {
		...
		if shouldUpdate {
			...
			resource.AddSupplier(NewS3BucketSupplier(provider.Runner().SubRunner(), factory, alertr))
		}
