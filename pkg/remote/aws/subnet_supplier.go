...
func NewSubnetSupplier(runner *parallel.ParallelRunner, client ec2iface.EC2API, alerter *alerter.Alerter) *SubnetSupplier {
...
func (s SubnetSupplier) Resources() ([]resource.Resource, error) {
	...
	if err != nil {
		handled := handleListAwsError(err, aws.AwsSubnetResourceType, s.alerter)
		if !handled {
			return nil, err
		}
