...
func NewEC2EipSupplier(runner *parallel.ParallelRunner, client ec2iface.EC2API, alerter *alerter.Alerter) *EC2EipSupplier {
...
func (s EC2EipSupplier) Resources() ([]resource.Resource, error) {
	...
	if err != nil {
		handled := handleListAwsError(err, resourceaws.AwsEipResourceType, s.alerter)
		if !handled {
			return nil, err
		}
